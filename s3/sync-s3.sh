...
function main() {
  ...
  LOCATIONS_LIST="/home/cris/.sync_s3_locations"
  for line in $(cat $LOCATIONS_LIST); do
    # Set space as the delimiter
    IFS='@'; #setting comma as delimiter
    read -a location <<< "$line"; #reading str as an array as tokens separated by IFS
    pretty_print "Syncing ${location[0]} to ${location[1]}";

    /usr/local/bin/aws s3 sync ${location[0]} ${location[1]}
  done
